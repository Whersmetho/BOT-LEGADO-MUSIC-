  ...
  async execute(message, args, client) {
    ...
    if (!queue) {
      return message.reply({
        embeds: [new EmbedBuilder().setColor('#E74C3C').setDescription('❌ No hay cola activa.')]
      });
    }
    ...
    const { allowed, reason } = canControl(message.member, queue, 'l!dc');
    if (!allowed) {
      return message.reply({
        embeds: [new EmbedBuilder().setColor('#E74C3C').setDescription(reason)]
      });
    }
    ...
    return message.reply({
      embeds: [new EmbedBuilder().setColor('#95A5A6').setDescription('👋 **Desconectado y cola limpiada.**')]
    });
  },
};
